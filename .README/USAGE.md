...
Runtime validation is added by defining a zod [object](https://github.com/colinhacks/zod#objects) and passing it to `sql.type` tagged template, e.g.
...
Now you need to pass this zod object to `sql.type`:

```ts
const persons = await connection.any(sql.type(personObject)`
  SELECT id, name
  FROM person
`);
```

With this information, Slonik guarantees that every member of `persons` is an object that has properties `id` and `name`, which are number and string respectively.

Furthermore, you can now infer the TypeScript type of the query result. There are couple of ways of doing it:
