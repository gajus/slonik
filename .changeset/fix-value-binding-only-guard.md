...
value binding."). Since the migration to Slonik's internal `$slonik_N`
placeholders, the guard ran against the not-yet-converted SQL and compared it to
`$1`, while the tag actually produces `$slonik_1` at that point — so the check
